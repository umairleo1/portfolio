...
const Experience: React.FC = () => {
  ...
  const getTechSkills = (company: string) => {
    const skillMap: { [key: string]: string[] } = {
      'UNICEF UK': ['Python', 'SQL', 'AWS Lambda', 'Salesforce', 'Snowflake', 'GitLab CI'],
      'University of Greenwich': ['Python', 'LangGraph', 'Machine Learning', 'SQL', 'Data Mining'],
      'SprintX': ['React 18', 'React Query', 'CircleCI', 'Jenkins', 'SonarQube', 'Jest'],
      'Frizhub': ['React Native', 'AWS', 'GCP', 'Docker', 'Jenkins', 'Terraform']
    };
    return skillMap[company] || ['JavaScript', 'TypeScript', 'React', 'Node.js'];
  };
      ...
      <div className="container">
        ...
        <div className="experience__accordion">
          {experience.map((item, index) => (
            <div key={index} className="experience__card" data-aos="fade-up" data-aos-delay={index * 100}>
              <div 
              ...
              <div className={`experience__card-content ${expandedItems.includes(index) ? 'expanded' : ''}`}>
                <div className="experience__content-inner">
                  <div className="experience__location">
                    {renderIcon(HiLocationMarker, { className: 'experience__icon' })}
                    {item.location}
                  </div>
                  
                  <div className="experience__website">
                    {renderIcon(HiExternalLink, { className: 'experience__icon' })}
                    <a href={getCompanyWebsite(item.company)} target="_blank" rel="noopener noreferrer">
                      {item.company} Website
                      {renderIcon(HiExternalLink, { className: 'external-link-icon' })}
                    </a>
                  </div>
